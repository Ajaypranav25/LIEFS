...
def run_benchmark(
    ...
    print(f"  CPU:       {hw.cpu_model} ({hw.cpu_physical_cores} Physical / {hw.cpu_logical_cores} Logical Cores)")
    print(f"  RAM:       {hw.ram_total_gb} GB Total ({hw.ram_available_gb} GB Available)")
    print(f"  GPU:       {hw.gpu_name} (Compute Capability: {hw.gpu_compute_capability or 'N/A'})")
    print(f"  VRAM:      {hw.vram_total_mb:.1f} MB Total ({hw.vram_free_mb:.1f} MB Free)")
    ...
    print(f"  Architecture: {meta.num_layers} Layers | {meta.num_attention_heads} Q Heads | {meta.num_kv_heads} KV Heads | Head Dim {meta.head_dim}")
    print(f"  Memory Footprint: {meta.memory_footprint_mb:.1f} MB (FP16 Estimate: {meta.fp16_vram_estimate_mb:.1f} MB)\n")
    ...
    test_prompt = "Explain how transformer self-attention works step by step in plain terms."
    ...
    if paged_engine:
        ...
        results.append({
            ...
            "speedup": round(m_naive.total_time_ms / max(1.0, m_paged.total_time_ms), 2),
        })
    ...
    if not quick:
        ...
        for b_size in [1, 2, 4, 8]:
            ...
            print(f"    Batch Size {b_size:>2}: {agg_throughput:>6.1f} tok/s across {total_toks} tokens")
    ...
    print("=" * 102)
    print(f"| {'Engine / Optimization':<30} | {'Throughput':<12} | {'TTFT':<10} | {'TPOT':<10} | {'VRAM':<10} | {'Bandwidth':<10} | {'Speedup':<8} |")
    print("|" + "-" * 32 + "|" + "-" * 14 + "|" + "-" * 12 + "|" + "-" * 12 + "|" + "-" * 12 + "|" + "-" * 12 + "|" + "-" * 10 + "|")
    for r in results:
        print(f"| {r['name']:<30} | {r['throughput_tok_s']:>8.1f} tok/s | {r['ttft_ms']:>7.1f} ms | {r['tpot_ms']:>7.1f} ms | {r['peak_vram_mb']:>7.1f} MB | {r['memory_bandwidth_gbs']:>6.1f} GB/s | {r['speedup']:>6.2f}x |")
    print("=" * 102)

    print("\n" + "#" * 70)
    print(f"  LIEFS COMPUTER PERFORMANCE SCORE:  {score_data['score']} POINTS  [{score_data['badge']}]")
...
def main():
    parser = argparse.ArgumentParser(description="LIEFS Universal Computer & Model Benchmark")
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL_NAME, help="Hugging Face repo ID or local model path")
    parser.add_argument("--device", type=str, default="auto", choices=["auto", "cuda", "cpu"], help="Compute device")
    parser.add_argument("--precision", type=str, default="float16", choices=["float16", "bfloat16", "float32"], help="Weight precision")
    parser.add_argument("--token", type=str, default=None, help="Hugging Face API token")
    parser.add_argument("--quick", action="store_true", help="Run short 32-token benchmark")
    parser.add_argument("--export", type=str, default=None, help="Output JSON path to export benchmark results")
